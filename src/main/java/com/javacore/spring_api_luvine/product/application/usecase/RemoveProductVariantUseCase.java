...
public class RemoveProductVariantUseCase {
    ...
    public void execute(UUID productPublicId, UUID variantPublicId) {
        log.info("event=remove_product_variant_attempt productId={} variantId={}",
                productPublicId, variantPublicId);

        Product product = productRepository.findByPublicId(productPublicId)
                .orElseThrow(() -> {
                    log.warn("event=remove_product_variant_rejected reason=product_not_found productId={}",
                            productPublicId);
        ...
        log.info("event=remove_product_variant_completed productId={} variantId={}", productPublicId, variantPublicId);
    }
}