...
public class DeactivateCategoryUseCase {
    ...
    public void execute(UUID categoryPublicId) {
        log.info("event=deactivate_category_attempt publicId={}", categoryPublicId);

        Category category = categoryRepository.findByPublicId(categoryPublicId)
                .orElseThrow(() -> {
                    log.warn("event=deactivate_category_rejected reason=category_not_found publicId={}",
                            categoryPublicId);
        ...
        log.info("event=deactivate_category_completed publicId={}", categoryPublicId);
    }
}