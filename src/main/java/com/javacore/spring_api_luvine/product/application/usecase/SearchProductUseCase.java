...
public class SearchProductUseCase {
    ...
    public Page<ProductSummaryResponse> execute(SearchProductRequest request, Pageable pageable) {
        log.info("event=search_product_attempt page={} size={}", pageable.getPageNumber(), pageable.getPageSize());

        Specification<Product> specification = ProductSearchSpecifications.build(request);

        Page<Product> products = productRepository.findAll(specification, pageable);

        log.info("event=search_product_completed totalElements={} totalPages={}",
